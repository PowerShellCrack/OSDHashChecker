<#
.SYNOPSIS
    Process hashes in MDT deploymentshare and store them

.DESCRIPTION
   hashes are performed on key files and the digest is saved to the deploymentshare
   Designed for OEM Media deployments for validating content using LTIHashCheckUI.ps1
...
.PARAMETER StoreType
    OPTIONS: StoreHash|ExportHash
    DEFAULT StoreHash.

    StoreHash  --> Stored the hash digest to custom properties within the customesettings.ini
    ExportHash --> Exports hashed to clixml at root of deployment share

.PARAMETER TaskSequenceID
    STRING. Specifies the TS.xml to hash
...
.EXAMPLE
    .\LTIHashStoreUI.ps1 -WorkingPath "C:\DeploymentShare" -StoreType StoreHash
    -----------
    Description
    Stores the hash values in properties to the C:\DeploymentShare\Control\customesettings.ini
    -----------
    Example
    [Settings]
    Priority=Default
...
.EXAMPLE
    .\LTIHashStoreUI.ps1 -WorkingPath "C:\DeploymentShare" -StoreType ExportHash
    -----------
    Description
    Saves the hash values to files in C:\DeploymentShare
...
.EXAMPLE
    .\LTIHashStoreUI.ps1 -WorkingPath "C:\DeploymentShare" -StoreType StoreHash -Title 'Updating Hashes' -ShowStatusUI
    -----------
    Description
    Stores the hash values in properties to the C:\DeploymentShare\Control\customesettings.ini  while displaying a
    status UI with ''Updating Hashes' as title. Script attempts finds property TaskSequenceID in customesettings.ini

.EXAMPLE
    .\LTIHashStoreUI.ps1 -WorkingPath "C:\DeploymentShare" -Title 'Updating Hashes' -TaskSequenceID WIN10_EUH -ShowStatusUI
    -----------
    Description
    Stores the hash values in properties to the C:\DeploymentShare\Control\customesettings.ini (by default) while displaying a
    status UI with 'Updating Hashes' as title. TaskSequenceID is specified and will use the ID to find the attached operating system file
...
Switch($StoreType){
    'ExportHash'  {$MsgPastVerb = 'Exported'}
    'StoreHash'    {$MsgPastVerb = 'Stored'}
}
...
Else{
    ...
    Measure-Command {
        ...
        Get-Job -Name WIMHasher | Remove-job
...
Measure-Command {
    If($StoreType -eq 'ExportHash')
    {
        ...
        Invoke-StatusUpdate -Message ("[{0} of {1}] :: Hashing the collection of file names exported to [{2}]. Please wait..." -f $i,$s,$TempFileList) `
                        -Step ($stepCounter++) -MaxStep $script:Maxsteps -UpdateUI:$ShowStatusUI `
                        -UpdateTextElement @{'Hash_DeployFiles_Text'='Hashing files...'} `
                        -Outhost -ShowProgress
...
switch($StoreType){
    'ExportHash'  {
                        #working path should be the first path as source to compare from
                        Write-host ("[{0} of {1}] :: Exporting file and folder to xml [{2},{3}].  " -f $i,$s,$ExportedFileList,$ExportedFolderList) -NoNewline -ForegroundColor Green
                        #store current file names in a list to the deploy folder for comparaison
                        Copy-item $TempFileList $ExportedFileList -Force -ErrorAction SilentlyContinue | Out-Null
                        #store current list of the working folder structure to the deploy folder for comparaison
                        Copy-item $TempFolderList $ExportedFolderList -Force -ErrorAction SilentlyContinue | Out-Null

                        Copy-item $TempTSHash $ExportedTSHash -Force -ErrorAction SilentlyContinue | Out-Null

                        Copy-item $TempTSHash $ExportedWIMHash -Force -ErrorAction SilentlyContinue | Out-Null

                        Invoke-StatusUpdate -Message ('Completed Exporting files') -Step $script:Maxsteps -MaxStep $script:Maxsteps -UpdateUI:$ShowStatusUI `
                                -Outhost -ShowProgress -DisplayColor Green
                    }
    ...
    'StoreHash'    {
                        ...
                        Write-host ("[{0} of {1}] :: Updating file [{2}] with {3} hashes..." -f $i,$s,$CSINI,$DeployHashes.count)
                        Try{
                            #Grab the Properties in [Settings] section
                            [System.Collections.ArrayList]$CSIniProperties = ((Get-IniContent -FilePath $CSINI).Settings.Properties) -split ','
                            #Add hash properties to [Settings] section
                            $DeployHashes.GetEnumerator() | Foreach-Object {
                                If($_.key -notin $CSIniProperties){
                                    Write-Host ("Added Property: [{0} = {1}]" -f $_.key,$_.Value) -ForegroundColor Green
                                    $CSIniProperties.Add($_.key) | Out-Null
                                }Else{
                                    Write-Host ("Updated Property: [{0} = {1}]" -f $_.key,$_.Value) -ForegroundColor Yellow
                                }
                            }
                            $PropertyNameKey = @{'Properties'=($CSIniProperties -join ',')}
                            Set-IniContent -FilePath $CSINI -NameValuePairs $PropertyNameKey -Sections '[Settings]' | Out-IniFile $CSINI -Force -Pretty -Encoding ASCII
                            #Add Hash values to [Default] section
                            Set-IniContent -FilePath $CSINI -NameValuePairs $DeployHashes -Sections '[Default]' | Out-IniFile $CSINI -Force -Pretty -Encoding ASCII
                        }
                        Catch{
                            Write-host ("Unable to update file [{0}]. {1} " -f $CSINI,$_.Exception.message) -NoNewline -ForegroundColor Red
                            Write-host ("Exporting file and folder to xml [{0},{1}].  " -f $ExportedFileList,$ExportedFolderList) -NoNewline -ForegroundColor Green
                            #store current file names in a list to the deploy folder for comparaison
                            Copy-item $TempFileList $ExportedFileList -Force -ErrorAction SilentlyContinue | Out-Null
                            #store current list of the working folder structure to the deploy folder for comparaison
                            Copy-item $TempFolderList $ExportedFolderList -Force -ErrorAction SilentlyContinue | Out-Null
                        ...
                        Invoke-StatusUpdate -Message ('Completed Storing hashes') -Step $script:Maxsteps -MaxStep $script:Maxsteps -UpdateUI:$ShowStatusUI `
                                -Outhost -ShowProgress -DisplayColor Green
                    }
